...
router = APIRouter(
    prefix="/sub-product-type",
    tags=["sub-product-type"]
    )
...
async def get_all_or_by_product_type(productType:str| None = None):
    ...
    try:
        ...
        query = f"select row_to_json(row) from ( select id, descripcion from sub_tipo_producto {filter} order by descripcion) row"
        cursor = conn.cursor()
