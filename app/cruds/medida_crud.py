...
@router.get("/{idProductType}")
async def get_by_product_type(idProductType:str):
    dict_json = []
    try:
        conn = utils.conexion_postgres(host,port,db,usr,pwd)
        
        query = f"select row_to_json(row) from (select id, descripcion from medida where id_tipo_producto = '{idProductType}' order by descripcion ) row"
        cursor = conn.cursor()
        cursor.execute(query)
        print('Query ejecutado')
        if cursor.rowcount > 0:
